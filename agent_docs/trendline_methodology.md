...
### 8.2 Multi-Filter Breakout Confirmation

A break is confirmed when **at least 2 of the following 3 filters** are satisfied:
...
**Filter 3 — Volume Filter**: Volume on breakout bar must be >= `VOLUME_BREAKOUT_MULTIPLIER × average_volume(20)`.
...
- `volume_score` = average volume at touches / overall average volume (normalized)
...
- **Volume profile analysis** at S/R zones (Market Profile / Volume-at-Price)
